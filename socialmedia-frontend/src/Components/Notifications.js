...
import "../CssDesigns/Notification.css";
const Notification = ({ token, userId , closeNotifications }) => {
    ...
    useEffect(() => {
      ...
      const fetchNotifications = async () => {
        try {
          const response = await fetch(`https://localhost:7069/api/Notifications/myNotifications`, {
            headers: {
  ...
  >
    ...
    <div className='kontenti' >
      ...
      <button onClick={() => markNotificationAsRead(notification.notificationId)}>Read</button>
    </div>
  </div>
))}
            </div>
