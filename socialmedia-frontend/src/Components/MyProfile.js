...
import '../CssDesigns/MyProfile.css';


const MyProfile = ({ userId, username, token, postId }) => {
...
const fetchPosts = async () => {
  try {
    ...
    const response = await fetch(`https://localhost:7069/api/posts/myposts/?userId=${userId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const handleCreatePost = async () => {
  try {
    // Create a complete Post object to send to the server
    const postData = {
      UserId: userId,
    ...
    if (response.ok) {
      ...
      if (responseData.id) {
        // Fetch the imageObjectUrl using the fetchImageObjectUrl function
        const imageObjectUrl = await fetchImageObjectUrl(imageUrl);
        ...
        const newPost = {
          ...
          createdAt: new Date().toISOString(),
          imageUrl: imageUrl,
          imageObjectUrl: imageObjectUrl, // Include the imageObjectUrl
        };

        // Update the imageUrlsMap with the new post's image URL
        

        // Temporarily add the new post to the top of the state
        setPosts((prevPosts) => [newPost, ...prevPosts]);

        console.log("arber", newPost)
        ...
        // Fetch the updated list of posts to include the newly created post
        fetchPosts();
      }
    } else {
...
const [profilePictureUrls3, setProfilePictureUrls3] = useState({});
...
const fetchMyProfilePicture2 = async (userData) => {
  try {
    if (userData.profilePicture) {
      const profilePictureUrl = await fetchPhotoByUrl(userData.profilePicture);
      if (profilePictureUrl) {
        setProfilePictureUrls3({ [userData.userId]: profilePictureUrl });
      } else {
        console.error("Failed to fetch image URL for the current user");
      }
    }
...
const [newProfileImage, setNewProfileImage] = useState(null);
  ...
  const handleUpdateProfilePicture = async () => {
    try {
      ...
      // Assuming newProfileImage is a File object representing the selected image
      ...
      const response = await fetch(`https://localhost:7069/api/Profile/profile-picture`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        // Assuming this endpoint returns the new profile picture URL
        const updatedProfilePictureUrl = await response.json();
  
        // Update the profilePictureUrls3 object with the new URL
        profilePictureUrls3[userInfo.userId] = updatedProfilePictureUrl;
  ...
  <img
    src={newProfileImage ? URL.createObjectURL(newProfileImage) : profilePictureUrls3[userInfo.userId]}
    alt={`Profile`}
  ...
  {newProfileImage && (
    <button className="custom-file-upload" onClick={handleUpdateProfilePicture}>Update Image</button>
  )}

<a style={{ fontSize: '30px', fontWeight: 'bold', marginLeft:'5%' }}>{userInfo.name} {userInfo.surname}</a>

</div>
      <div className="post-form" style={{width:'61%', marginLeft:'-2%'}}>
      <h2 style={{marginLeft:'37%', marginTop:'0%'}}>Create a Post</h2>
      <div className="post-editor">
        ...
        />
        <input type="file"  ref={fileInputRef}  onChange={handleAttachment} accept="image/*" />
        {imageUrl3 && <img src={imageUrl3} alt="Select" style={{width:'20vw',  height:'20vh' }}/>}
        {isImageSelected || imageUrl3 ? (
